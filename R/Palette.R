# 2025.05.18修改了DownSample函数关于ATAC的降维SVD的计算函数。实验1中的监督算法结果使用的是之前版本的函数
# 2025.05.18添加了Run_Multi_SVD_s函数，修改了Find_SubSpace3_s函数关于ATAC基于SVD预降维的代码。实验1中的监督算法结果对ATAC的降维基于随机投影，此修改不影响结果
# code from Palette_Function0219.R
# library(dplyr)
# library(patchwork)
# library(tidyverse)
# library(RSpectra)
...
#' @param sparce Whether use sparse format ('dgCMatirx' class). (default TRUE)
#'
...
#' @param modals A character represents the modality information for the data matrix.
#' @param filter Whether to filter the data.
#' @param min.cells Include features detected in at least this many cells. Use only if \code{filter = TRUE}.
#' @param min.features Include cells where at least this many features are detected. Use only if \code{filter = TRUE}.
#' @param slot Which slot ('Raw' or 'data') to put the \code{count}.
#' @param sparce Whether to use sparse format ('dgCMatirx' class). (default TRUE)
#'
  ...
  for(i in 1:length(modal)){
    ...
    for(j in 1:length(input.data)){
      snn <- Seurat::FindNeighbors(t(input.data[[j]]),
                            ...
                            verbose = verbose)$snn
      out_i[[j]] <- Seurat::FindClusters(snn,
...
Find_SubSpace_s <- function(object,
  ...
  if(nrow(data.mat) > RP_thresh){
    if(pre.reduce){
      ...
      rownames(V) <- features
      data_res <- as.matrix(Reduce(cbind,data_res))[as.integer(pre_dims[[1]]),]
      V <- V[,as.integer(pre_dims[[1]])]
      colnames(data_res) <- colnames(data.mat)
      rownames(data_res) <- paste0("dim_",seq(nrow(data_res)))
      colnames(V) <- paste0("dim_",seq(nrow(data_res)))
      rm(data.mat)
      gc(verbose = F)
    }else{
      # 执行随机投影
...
#' @param L2.norm Logical; whether to apply L2 normalization to the final embedding (default: TRUE).
#' @param seed Integer random seed for reproducibility (default: 123L).
#' @param pre.reduce Logical; whether to perform pre-dimensionality reduction before random projection (default: FALSE).
...
#' @examples
#' \dontrun{
#' # Run unsupervised subspace embedding
#' obj <- Find.Subspace(object, modal = c("RNA","ATAC"), sub.dims = list(1:50, 1:30))
#'
#' # Run supervised version
#' obj <- Find.Subspace(object, modal = "RNA", supervised = TRUE, sub.dims = list(1:40))
#' }
#'
#' @import Matrix
#' @import Seurat
...
#' @param k Integer. Parameter controlling local connectivity during inference.
...
Run.Palette <- function(object,
  ...
  message('Palett integration Done! \n')
...
run_Harmony = function(batches, cell_name,
  ...
  t1 = Sys.time()
  if(method == 'TF-IDF'){
    batch_seurat = Seurat::RunHarmony(object = batch_seurat,
                                      group.by.vars = 'batch',
                                      theta = theta,
                                      dims.use = 1:out.npcs,
...
#'
#' @param datalist A list of expression matrices or data frames for each sample.
#' @param modal A character vector indicating the modality of each sample.
#' @param batch A character or factor vector indicating the batch of each sample.
#' @param q_or_ref A character vector indicating whether each sample is 'query' or 'ref'.
